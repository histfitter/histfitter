...
Double_t 
StatTools::GetProbFromSigma( Double_t nsigma )
{
  Double_t p = -1.0;
...
//     StatTools::SRootFinder rootFinder( &GetSigma, 1e-45, 1e-13, 500);
//     p = rootFinder.Root( nsigma );
     p=1.0;
     if (sigsign){
        p=0;
     }
  ...
  // return TMath::Erfc( nsigma / TMath::Sqrt(2.0) );
}


//________________________________________________________________________________________________
Double_t 
StatTools::GetSigma( Double_t p )
{
   ...
   Double_t nsigma = 0;
   if (p > 1.0e-16) nsigma = TMath::ErfInverse( 1.0 - 2.0 * p )*TMath::Sqrt(2.0);
   else if (p > 0) {
     // inversion is not possible, search numerically for the root
     // SRootFinder rootFinder( &GetProbFromSigma, 0, 300 );
     // nsigma = rootFinder.Root( p );
...
Double_t 
StatTools::DmLogL_PA( Double_t nObs, Double_t nb, Double_t sig ) 
{
...
Double_t
StatTools::GetDLL( Double_t nObs, Double_t nb, Double_t sig )
{
   Double_t sig2 = sig*sig;
   Double_t sig4 = sig2*sig2;
   Double_t nb2  = nb*nb;
...
Double_t 
StatTools::GetNSigma( Double_t nObs, Double_t nb, Double_t sig )
{
...
Double_t
StatTools::GetSimpleP1( Double_t nObs, Double_t nEvtExp, Double_t sumErr )
{
...
Double_t
StatTools::FindS95( Double_t nObs, Double_t nEvtExp, Double_t sumErr, Double_t signald, Double_t signalu, Double_t dll95, Double_t tol )
{
...
Double_t
StatTools::FindSNSigma( Double_t nEvtExp, Double_t sumErr, Double_t nsigma3, Double_t signald, Double_t signalu, Double_t tol )
{
  ...
  if ( nsigmad<nsigma3 && nsigmau>nsigma3 ) {
    if (nsigmau-nsigmad<tol) { return (signald+signalu)/2. ; }
...
Double_t
StatTools::FindSignal( Double_t nObs, Double_t nEvtExp, Double_t sumErr, Double_t signald, Double_t signalu, Double_t pValue, Double_t tol )
{
...
Double_t 
StatTools::FindXS95( Double_t nObs, Double_t nBkgExp, Double_t nBkgErr, Double_t signalEff, Double_t lumi, Double_t pValue )
{
  if ( signalEff<=0 || lumi<=0 ) return -1;
  if ( pValue<0 || pValue>1 ) return -1; 
...
Double_t 
StatTools::FindXSNSigma( Double_t nBkgExp, Double_t nBkgErr, Double_t signalEff, Double_t lumi, Double_t nsigma3, Double_t signalu ) 
{
  if ( signalEff<=0 || lumi<=0 ) return -1;
  if ( nsigma3<0 ) return -1;

  double snsigma3 = StatTools::FindSNSigma( nBkgExp, nBkgErr, nsigma3, 0.0, signalu );
  if (snsigma3<0) return -1;
...
Double_t 
StatTools::FindXSNSigma( Double_t nBkgExp1, Double_t nBkgErr1, Double_t signalEff1, Double_t nBkgExp2, Double_t nBkgErr2, Double_t signalEff2,
                         Double_t lumi, Double_t nsigma3, Double_t xsectiond, Double_t xsectionu, Double_t tol ) 
{
  ...
  if ( nsigmad<nsigma3 && nsigmau>nsigma3 ) {
    if (nsigmau-nsigmad<tol) { return (xsectiond+xsectionu)/2. ; }
...
Double_t
StatTools::FindXS95( Double_t nObs1, Double_t nBkgExp1, Double_t nBkgErr1, Double_t signalEff1, 
                     ...
                     Double_t lumi, Double_t dll95, Double_t xsectiond, Double_t xsectionu, Double_t tol )
{
  ...
  if ( dlld<dll95 && dllu>dll95 ) {
    if (xsectionu-xsectiond<tol) { return (xsectionu+xsectiond)/2. ; }
