...
void RooPlot::printMultiline(std::ostream& os, Int_t /*content*/, Bool_t verbose, TString indent) const
{
  ...
  if(verbose) {
    ...
    Int_t i=0 ;
    while((obj= _iterator->Next())) {
      os << deeper << "[" << i++ << "] (Options=\"" << _iterator->GetOption() << "\") " << obj << " ";
...
//_____________________________________________________________________________
RooHist* RooPlot::residHist(const char* histname, const char* curvename, bool normalize, bool useAverage,  bool returnRatio) const
{
  ...
  return hist->makeResidHist(*curve,normalize,useAverage,returnRatio) ;
}
