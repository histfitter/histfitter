...
ClassImp(ExpandedRooFitResult);

//_____________________________________________________________________________
// ExpandedRooFitResult::ExpandedRooFitResult(){
// }

//_______________________________________________________________________________________
ExpandedRooFitResult::ExpandedRooFitResult(RooFitResult* origResult, RooArgList extraPars){
  ...
  for (Int_t i = 0 ; i < allPars.getSize() ; i++) {
    ...
    //cout << " i = " << i << " parameter = " << par->GetName() << endl;
    if (par->isConstant()) {
      //  cout << "   constant parameter = " << par->GetName() << endl;
  ...
  // Save all (original+extra) floating/const parameters to new ExpandedRooFitResult
  ...
  for (Int_t ic=0; ic<saveFloatFinalList.getSize(); ic++) {
	...
	if(ic==ii &&  par1->InheritsFrom("RooRealVar") &&  par2->InheritsFrom("RooRealVar")){
	  corrs(ic,ii) = 1.;
	  covs(ic,ii) = ((RooRealVar*) par1)->getError() * ((RooRealVar*) par2)->getError() ;
	}
	else{
	  corrs(ic,ii) = 1e-300; //
	  covs(ic,ii) = 1e-300; //00000000001;
	}
      }
    }
  }
  
  //  for(unsigned int i = 0; i<globalCC.size(); i++){ cout << "globalCC[" <<i<<"] = " << globalCC[i] << endl;}
  //   corrs.Print();
  //   covs.Print();
...
ExpandedRooFitResult::ExpandedRooFitResult(RooArgList extraPars){
  
  TString name,title ;
  name = "ExpandedRooFitResult";
  title = "ExpandedRooFitResult";
