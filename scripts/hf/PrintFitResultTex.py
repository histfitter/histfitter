...
%%'''
  ...
  for name in m_listofkeys:
      ...
      phantom = ""
      if m[name][4]>=0: phantom = r"\phantom{-}"
      if str("%.2f" %m[name][4])=="0.00": phantom = r"\phantom{-}"
...
\\end{{center}}
\\caption[Fit results]{{
Floating fit parameters for the analysis involving signal region {}, before (left) and after (right) the background-only fit. The quoted fit errors come from HESSE.
\\label{{fitparameters.{}}}}}
\\end{{table}}
%'''.format(sr,sr)
