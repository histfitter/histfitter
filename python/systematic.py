...
class SystematicBase:
    ...
    def FillUpDownHist(
        ...
        if self.method == 'userNormHistoSys' or self.method\
             == 'normHistoSys':
            if not 'h' + sam.name + self.name + lowhigh + normString\
                 + 'Norm' in abstract.hists.keys():
                if abstract.readFromTree:
                    abstract.hists['h' + sam.name + self.name + lowhigh
                                    + normString + 'Norm'] = TH1F('h'
                             + sam.name + self.name + lowhigh
                             + normString + 'Norm', 'h' + sam.name
                             + self.name + lowhigh + normString + 'Norm'
                            , 1, 0.5, 1.5)
                    abstract.chains[abstract.prepare.currentChainName].Project('h'
                             + sam.name + self.name + lowhigh
                             + normString + 'Norm', normCuts,
                            abstract.prepare.weights + ' * (' + normCuts
                             + ')')
                else:

                    abstract.hists['h' + sam.name + self.name + lowhigh
                                    + normString + 'Norm'] = None
                    abstract.prepare.addHisto('h' + sam.name + self.name
                             + lowhigh + normString + 'Norm')
    ...
    def tryAddHistos(
        ...
        if abstract.verbose > 1:
            print '!!!!!! adding histo', 'h' + sam.name + self.name\
                 + highorlow + regionString + '_obs_'\
...
class TreeWeightSystematic(SystematicBase):
    ...
    def PrepareWAHforWeight(
        ...
        for highorlow in highandlow:
            ...
            TreeWeightSystematic.tryAddHistos(
                self,
                highorlow,
                regionString,
                normString,
            ...
            TreeWeightSystematic.FillUpDownHist(
                self,
                highorlow,
                regionString,
                normString,
    ...
    def PrepareWeightsAndHistos(
        ...
        if self.type == 'weight':
            TreeWeightSystematic.PrepareWAHforWeight(
                self,
                regionString,
                normString,
                normCuts,
        ...
        if self.type == 'tree':
            TreeWeightSystematic.PrepareWAHforTree(
                self,
                regionString,
                normString,
                normCuts,
