...
class Measurement(object):
    ...
    def __str__(self):
        ...
        measurementString = \
            '  <Measurement Name="%s" Lumi="%g" LumiRelErr="%g" BinLow="%d" BinHigh="%d" Mode="%s" ExportOnly="%s">\n'\
             % (
            self.name,
            self.lumi,
            self.lumiErr,
        ...
        for (param, setting) in self.paramSettingDict.iteritems():
            if setting[0]:
                if not setting[1] == None:
                    ...
                    measurementString += \
                        '    <ParamSetting Const="True">%s</ParamSetting>\n'\
                         % param
            else:
                if not setting[1] == None:
                    measurementString += \
                        '    <ParamSetting Const="False" Val="%g">%s</ParamSetting>\n'\
                         % (setting[1], param)
                else:
                    measurementString += \
                        '    <ParamSetting Const="False">%s</ParamSetting>\n'\
                         % param
        for (param, constraint) in self.constraintTermDict.iteritems():
            if not constraint[1] == None:
                ...
                measurementString += \
                    '    <ConstraintTerm Type="%s">%s</ConstraintTerm>\n'\
                     % (constraint[0], param)
