...
class ChannelXML(object):
    ...
    def ConstructorInit(self, prefix):
        self.prefix = prefix
        self.xmlFileName = 'config/' + self.prefix + '_'\
             + self.channelName + '.xml'
        return
    ...
    def addDiscoverySamples(
        ...
        for (iSR, sr) in enumerate(srList):
            ...
            histoName = 'h' + sigSample.name + 'Nom_' + sr + '_obs_'\
                 + self.variableName.replace('/', '')
            self.getSample('DiscoveryMode_%s'
                            % sr).setHistoName(histoName)
            configMgr.hists[histoName] = TH1F(histoName, histoName,
                    len(srList), 0., float(len(srList)))
    ...
    def addData(self, dataName):
        ...
        if len(self.dataList):
            raise IndexError('Channel already has data '
                              + str(self.dataList))
    ...
    def addPseudoData(
        ...
        for iBin in xrange(histo.GetNbinsX() + 1):
            for inputHisto in addInputHistoList:
                histo.SetBinContent(iBin + 1,
                                    TMath.Nint(histo.GetBinContent(iBin
                                     + 1))
                                     + TMath.Nint(inputHisto.GetBinContent(iBin
                                     + 1)))
        for iBin in xrange(histo.GetNbinsX() + 1):
            histo.SetBinError(iBin + 1,
                              TMath.Sqrt(histo.GetBinContent(iBin + 1)))
    ...
    def addSystematic(self, syst):
        ...
        if syst.name in self.systDict.keys():
            raise Exception('Attempt to overwrite systematic %s in Channel %s'
                             % (syst.name, self.name))
        else:
...
'''\
        ...
        for data in self.dataList:
            if len(data[2]):
                self.writeString += \
                    '''  <Data HistoName="%s" InputFile="%s" HistoPath="%s" />

'''\
                     % (data[1], data[0.], data[2])
            else:
                self.writeString += \
                    '''  <Data HistoName="%s" InputFile="%s" />

'''\
                     % (data[1], data[0.])
        if self.hasStatConfig:
            self.writeString += \
                '''  <StatErrorConfig RelErrorThreshold="%g" ConstraintType="%s"/>
