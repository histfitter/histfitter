...
class FitWorkspace(object):
    ...
    def initialize(self):
                ...
                for chan in self.channels:
                    for s in chan.sampleList:
                        if s.name == self.signalSample:
                            found = True
                if not found:
                    print "WARNING signal sample %s is not contained in sampleList of FitWorkspace %s or its daughter channels"%(self.signalSample,self.name)
        ...
        for chan in self.channels:
            ...
            if self.validationChannels.__contains__(chanName):
                ...
                nFound+=1
            if nFound==0:
                print "WARNING FitWorkspace: %s, Channel: %s --> SR/CR/VR undefined"%(self.name,chanName)
            if nFound>1:
                print "WARNING FitWorkspace: %s, Channel: %s --> SR=%s CR=%s VR=%s is ambiguous"%(self.name,chanName,isSR,isCR,isVR)
    ...
    def addMeasurement(self,name,lumi,lumiErr):
        ...
        for meas in self.measurements:
            if meas.name == name:
                raise RuntimeError("Measurement %s already exists in FitWorkspace %s. Please use a different name."%(measObj.name,self.name))
            pass
    ...
    # TODO REWRITE TO WRITE TO A WORKSPACE
    def addChannel(self,variableName,regions,nBins,binLow,binHigh):
        ...
        for chan in self.channels:
            if chan.name == chanObj.name:
                raise RuntimeError("Channel %s already exists in FitWorkspace %s. Please use a different name."%(chanObj.name,self.name))
    ...
    def addSamples(self,input):
        """
        Add list (or single object) of pre-built samples to this FitWorkspace
        """
        ...
        for s in sampleList:
            # If the sample doesn't exist in FitWorkspace already then add it, else something has gone wrong
            if not s.name in [sam.name for sam in self.sampleList]:
                ...
                raise RuntimeError("Sample %s already defined in FitWorkspace %s" % (s.name,self.name))
    ...
    def getSample(self,name):
        ...
        for s in self.sampleList:
            if s.name == name:
                return s
        raise Exception("Sample with name %s not found in FitWorkspace %s"%(name,self.name))
    ...
    def addWeight(self,weight):
        ...
        if not weight in self.weights:
            ...
            raise RuntimeError("Weight %s already defined in FitWorkspace" % (weight,self.name))
    ...
    def removeWeight(self,weight):
        ...
        if weight in self.weights:
            ...
            raise RuntimeError("Weight %s does not exist in FitWorkspace %s" % (weight,self.name))
    ...
    def addSystematic(self,syst):
        ...
        if syst.name in self.systDict.keys():
            raise RuntimeError("Attempt to overwrite systematic %s in FitWorkspace %s" % (syst.name,self.name))
        else:
    ...
    def clearSystematics(self):
        """
        Remove all systematics from this FitWorkspace
        """
        self.systDict.clear()
        return
    ...
    def removeSystematic(self,syst):
        """
        Remove a single systematic from this FitWorkspace
        """
...
class Measurement(object):
    """
    Class to define measurements in the FitWorkspace
    """
...
class Channel(object):
    ...
    # TODO rewrite: should rely on top level workspace name
    ...
    # TODO rewrite to use the new function above that writes to a workspace
    def close(self):
        """
