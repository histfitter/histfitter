...
class ConfigManager(object):
    ...
    def __init__(self):
        ...
        self.lumiUnits = 1.  # 1=fb-1, 1000=pb-1, etc.
        ...
        self.muValGen = 0.  # mu_sig used for toy generation
    ...
    def addTopLevelXML(self, input, name=''):
        ...
        for tl in self.topLvls:
            if tl.name == newName:
                raise RuntimeError('TopLevelXML %s already exists in configManager. Please use a different name.'
                                    % newName)
            pass
    ...
    def removeTopLevelXML(self, name):
        for i in xrange(0., len(self.topLvls)):
            ...
            if tl.name == name:
                self.topLvls.pop(i)
                return
        print "WARNING TopLevelXML named '%s' does not exist. Cannot be removed."\
             % name
        return
    ...
    def getTopLevelXML(self, name):
        for tl in self.topLvls:
            if tl.name == name:
                return tl
        print "WARNING TopLevelXML named '%s' does not exist. Cannot be returned."\
             % name
        return 0.
            ...
            for chan in tl.channels:
                ...
                for sam in chan.sampleList:
                    if not sam.isData and not sam.isQCD\
                         and not sam.isDiscovery:
                        pass
                        for (name, syst) in self.systDict.items():
                            if not name\
                                 in chan.getSample(sam.name).systDict.keys():
            ...
            for chan in tl.channels:
                for sam in chan.sampleList:
                    ...
                    for reg in chan.regions:
                        ...
                        regString += reg
                    if sam.isData:
                        if not 'h' + sam.name + '_' + regString\
                             + '_obs_'\
                             + replaceSymbols(chan.variableName)\
                             in self.hists.keys():
                            self.hists['h' + sam.name + '_' + regString
                                     + '_obs_'
                                     + replaceSymbols(chan.variableName)] = \
                                None
                    elif sam.isQCD:
                        if not 'h' + sam.name + 'Nom_' + regString\
                             + '_obs_'\
                             + replaceSymbols(chan.variableName)\
                             in self.hists.keys():
                            self.hists['h' + sam.name + 'Nom_'
                                     + regString + '_obs_'
                                     + replaceSymbols(chan.variableName)] = \
                                None
                        if not 'h' + sam.name + 'High_' + regString\
                             + '_obs_'\
                             + replaceSymbols(chan.variableName)\
                             in self.hists.keys():
                            self.hists['h' + sam.name + 'High_'
                                     + regString + '_obs_'
                                     + replaceSymbols(chan.variableName)] = \
                                None
                        if not 'h' + sam.name + 'Low_' + regString\
                             + '_obs_'\
                             + replaceSymbols(chan.variableName)\
                             in self.hists.keys():
                            self.hists['h' + sam.name + 'Low_'
                                     + regString + '_obs_'
                                     + replaceSymbols(chan.variableName)] = \
                                None
                        if not 'h' + sam.name + 'Syst_' + regString\
                             + '_obs_'\
                             + replaceSymbols(chan.variableName)\
                             in self.hists.keys():
                            self.hists['h' + sam.name + 'Syst_'
                                     + regString + '_obs_'
                                     + replaceSymbols(chan.variableName)] = \
                                None
                        if not 'h' + sam.name + 'Stat_' + regString\
                             + '_obs_'\
                             + replaceSymbols(chan.variableName)\
                             in self.hists.keys():
                            self.hists['h' + sam.name + 'Stat_'
                                     + regString + '_obs_'
                                     + replaceSymbols(chan.variableName)] = \
                                None
                        ...
                        if not 'h' + sam.name + 'Nom_' + regString\
                            ...
                            self.hists['h' + sam.name + 'Nom_'
                                ...
                                None
                        for (name, syst) in \
                            chan.getSample(sam.name).systDict.items():
                            if not 'h' + sam.name + syst.name + 'High_'\
                                 + regString + '_obs_'\
                                 + replaceSymbols(chan.variableName)\
                                 in self.hists.keys():
                                self.hists['h' + sam.name + syst.name
                                         + 'High_' + regString + '_obs_'
                                         + replaceSymbols(chan.variableName)] = \
                                    None
                            if not 'h' + sam.name + syst.name + 'Low_'\
                                 + regString + '_obs_'\
                                 + replaceSymbols(chan.variableName)\
                                 in self.hists.keys():
                                self.hists['h' + sam.name + syst.name
                                         + 'Low_' + regString + '_obs_'
                                         + replaceSymbols(chan.variableName)] = \
                                    None
                            if syst.merged:
                                mergedName = ''
                                for s in syst.sampleList:
                                    mergedName += s
                                if not 'h' + mergedName + 'Nom_'\
                                     + regString + '_obs_'\
                                     + replaceSymbols(chan.variableName)\
                                     in self.hists.keys():
                                    self.hists['h' + mergedName + 'Nom_'
         + regString + '_obs_' + replaceSymbols(chan.variableName)] = \
    None
                                if not 'h' + mergedName + syst.name\
                                     + 'High_' + regString + '_obs_'\
                                     + replaceSymbols(chan.variableName)\
                                     in self.hists.keys():
                                    self.hists['h' + mergedName
         + syst.name + 'High_' + regString + '_obs_'
         + replaceSymbols(chan.variableName)] = None
                                if not 'h' + mergedName + syst.name\
                                     + 'Low_' + regString + '_obs_'\
                                     + replaceSymbols(chan.variableName)\
                                     in self.hists.keys():
                                    self.hists['h' + mergedName
         + syst.name + 'Low_' + regString + '_obs_'
         + replaceSymbols(chan.variableName)] = None
    ...
    def execute(self, topLvl):
        ...
        for (name, syst) in topLvl.systDict.items():
            if not name in systDict.keys():
                ...
                raise (Exception,
                       'Syst name %s already defined at global level. Rename for top level %s'
                       , (name, topLvl.name))
        ...
        for (iChan, chan) in enumerate(topLvl.channels):
            for (iSam, sam) in enumerate(chan.sampleList):
                chan.infoDict[sam.name] = [('Nom', self.nomName,
                        sam.weights, '')]
                if not sam.isData and not sam.isQCD:
            ...
            # ##depending on the systematic type: chan.infoDict[sam.name].append(...)
                        ...
                        self.appendSystinChanInfoDict(chan, sam,
                                systName, syst)
...
# Shouldn't have multiple imports... doesn't matter as singleton but good to check user isn't doing something strange
    ...
    def appendSystinChanInfoDict(
        ...
        if self.verbose > 1.:
            ...
            pass
        if syst.type == 'tree':
            chan.infoDict[sam.name].append((systName + 'High',
                    syst.high, sam.weights, syst.method))
            chan.infoDict[sam.name].append((systName + 'Low', syst.low,
                    sam.weights, syst.method))
        elif syst.type == 'weight':
            chan.infoDict[sam.name].append((systName + 'High',
                    self.nomName, syst.high, syst.method))
            chan.infoDict[sam.name].append((systName + 'Low',
                    self.nomName, syst.low, syst.method))
        else:
            chan.infoDict[sam.name].append((systName, syst.high,
                    syst.low, syst.method))
        return

    def addHistoSysforNoQCD(
        self,
        regionString,
        normString,
        normCuts,
        ...
        nomName = 'h' + sam.name + 'Nom_' + regionString + '_obs_'\
             + replaceSymbols(chan.variableName)
        highName = 'h' + sam.name + syst.name + 'High_' + regionString\
             + '_obs_' + replaceSymbols(chan.variableName)
        lowName = 'h' + sam.name + syst.name + 'Low_' + regionString\
             + '_obs_' + replaceSymbols(chan.variableName)

        if syst.method == 'histoSys':
            chan.getSample(sam.name).addHistoSys(
                syst.name,
                nomName,
                highName,
                lowName,
            ...
            print 'WARNING generating High HistoSys for %s syst=%s nom=%g high=%g low=%g'\
                 % (nomName, syst.name, nomIntegral, highIntegral,
                    lowIntegral)
            try:
            ...
            try:
                ...
                print 'WARNING generating Low HistoSys for %s syst=%s nom=%g high=%g low=%g'\
                     % (nomName, syst.name, nomIntegral, highIntegral,
                        lowIntegral)
                overallSystLow = 1.
            chan.getSample(sam.name).addOverallSys(syst.name,
                    overallSystHigh, overallSystLow)
            ...
            if not len(syst.high)\
                 == configMgr.hists[nomName].GetNbinsX()\
                 or not len(syst.low)\
                 == configMgr.hists[nomName].GetNbinsX():
                raise ValueError('High and low must both be the same as the binning in nominal for userHistoSys %s'
                                  % syst.name)
            if configMgr.hists[highName] == None:
                configMgr.hists[highName] = TH1F(highName, highName,
                        configMgr.hists[nomName].GetNbinsX(),
                        configMgr.hists[nomName].GetXaxis().GetXmin(),
                        configMgr.hists[nomName].GetXaxis().GetXmax())
                for iBin in \
                    xrange(configMgr.hists[nomName].GetNbinsX()):
                    configMgr.hists[highName].SetBinContent(iBin + 1.,
                            configMgr.hists[nomName].GetBinContent(iBin
                             + 1.) * syst.high[iBin])
            if configMgr.hists[lowName] == None:
                configMgr.hists[lowName] = TH1F(lowName, lowName,
                        configMgr.hists[nomName].GetNbinsX(),
                        configMgr.hists[nomName].GetXaxis().GetXmin(),
                        configMgr.hists[nomName].GetXaxis().GetXmax())
                for iBin in \
                    xrange(configMgr.hists[nomName].GetNbinsX()):
                    configMgr.hists[lowName].SetBinContent(iBin + 1.,
                            configMgr.hists[nomName].GetBinContent(iBin
                             + 1.) * syst.low[iBin])
            chan.getSample(sam.name).addHistoSys(
                syst.name,
            ...
            if not (syst.name, sam.name) in userNormDict.keys():
                ...
                userNormDict[(syst.name,
                             sam.name)].append((regionString, highName,
                        lowName, nomName))
            else:
                userNormDict[(syst.name,
                             sam.name)].append((regionString, highName,
            ...
            if syst.merged:
                ...
                for s in syst.sampleList:
                    mergedName += s
                nomMergedName = 'h' + mergedName + 'Nom_' + regionString\
                     + '_obs_' + replaceSymbols(chan.variableName)
                highMergedName = 'h' + mergedName + syst.name + 'High_'\
                     + regionString + '_obs_'\
                     + replaceSymbols(chan.variableName)
                lowMergedName = 'h' + mergedName + syst.name + 'Low_'\
                     + regionString + '_obs_'\
                     + replaceSymbols(chan.variableName)
                if sam.name in syst.sampleList:
                    syst.foundSample()
                    if self.hists[nomMergedName] == None:
                        self.hists[nomMergedName] = \
                            self.hists[nomName].Clone(nomMergedName)
                    else:
                        self.hists[nomMergedName].Add(self.hists[nomName])
                    if self.hists[highMergedName] == None:
                        self.hists[highMergedName] = \
                            self.hists[highName].Clone(highMergedName)
                    else:
                        self.hists[highMergedName].Add(self.hists[highName])
                    if self.hists[lowMergedName] == None:
                        self.hists[lowMergedName] = \
                            self.hists[lowName].Clone(lowMergedName)
                    else:
                        self.hists[lowMergedName].Add(self.hists[lowName])
                    if syst.isMerged():
                        chan.getSample(sam.name).addShapeSys(syst.name,
                                nomMergedName, highMergedName,
                                lowMergedName, syst.constraint)
                        syst.Reset()
                    chan.getSample(sam.name).shapeSystList.append((
                        systName,
                        nomMergedName + 'Norm',
                        syst.constraint,
                        '',
    ...
    def addHistoSysForQCD(
        ...
        self.prepare.addQCDHistos(sam, chan.useOverflowBin,
                                  chan.useUnderflowBin)
        if chan.variableName == 'cuts':
            nHists = len(chan.regions)
        else:
            nHists = chan.nBins

        prefixNom = 'h' + sam.name + 'Nom_' + regionString + '_obs_'\
             + replaceSymbols(chan.variableName)
        prefixHigh = 'h' + sam.name + 'High_' + regionString + '_obs_'\
             + replaceSymbols(chan.variableName)
        prefixLow = 'h' + sam.name + 'Low_' + regionString + '_obs_'\
             + replaceSymbols(chan.variableName)
        ...
        if chan.getSample(sam.name).qcdSyst == 'histoSys':
            ...
            try:
                ...
                print 'Error generating HistoSys for %s syst=%s nom=%g high=%g low=%g'\
                     % (nomName, 'QCDNorm_' + regionString + '_'
                         + replaceSymbols(chan.variableName),
                        nomIntegral, highIntegral, lowIntegral)
            chan.getSample(sam.name).addOverallSys('QCDNorm_'
                     + regionString + '_'
                     + replaceSymbols(chan.variableName),
                    overallSystHigh, overallSystLow)
            ...
            chan.getSample(sam.name).addShapeSys('QCDNorm_'
                     + regionString + '_'
                     + replaceSymbols(chan.variableName), prefixNom,
                    prefixHigh, prefixLow)
            chan.getSample(sam.name).shapeSystList.append((
                'QCDNorm_' + regionString + '_'
                     + replaceSymbols(chan.variableName),
                prefixNom + 'Norm',
                'data/' + configMgr.analysisName + '.root',
                '',
                '',
                '',
                '',
            ...
            for iBin in xrange(1., nHists + 1.):
                qcdSam = sam.Clone()
                qcdSam.name = sam.name + '_' + regionString + '_'\
                     + str(iBin)
                chan.addSample(qcdSam)
                chan.getSample(qcdSam.name).setWrite(True)
                chan.getSample(qcdSam.name).setHistoName('h' + sam.name
                         + 'Nom_' + regionString + '_obs_'
                         + replaceSymbols(chan.variableName) + '_'
                         + str(iBin))
                chan.getSample(qcdSam.name).addHistoSys(
                    'NormQCD' + regionString + '_'
                         + replaceSymbols(chan.variableName) + '_'
                         + str(iBin),
            ...
            raise Exception('Incorrect systematic method specified for QCD: %s'
                             % getSample(sam.name).qcdSyst)

    def setWeightsCutsVariable(
        self,
        chan,
        sam,
        regionString,
        ...
        if not sam.isData and not sam.isQCD:
            self.prepare.weights = str((self.lumiUnits
                     * self.outputLumi) / self.inputLumi)
        ...
        elif sam.unit == 'MeV' and chan.variableName.find('/') < 0.\
             and not chan.variableName.startswith('n'):
            self.prepare.var = chan.variableName + '/1000.'

    def addSampleSpecificHists(
        self,
        chan,
        sam,
        regionString,
        ...
        if sam.isData:
            self.prepare.addHisto('h' + sam.name + '_' + regionString
                                   + '_obs_'
                                   + replaceSymbols(chan.variableName),
                                  useOverflow=chan.useOverflowBin,
                                  useUnderflow=chan.useUnderflowBin)
            chan.addData('h' + sam.name + '_' + regionString + '_obs_'
                          + replaceSymbols(chan.variableName))
        elif not sam.isQCD and not sam.isDiscovery:

            if not len(sam.shapeFactorList):
                self.prepare.addHisto('h' + sam.name + 'Nom_'
                         + regionString + '_obs_'
                         + replaceSymbols(chan.variableName),
                        useOverflow=chan.useOverflowBin,
                        useUnderflow=chan.useUnderflowBin)
            else:
                self.hists['h' + sam.name + 'Nom_' + regionString
                            + '_obs_'
                            + replaceSymbols(chan.variableName)] = \
                    TH1F('h' + sam.name + 'Nom_' + regionString
                          + '_obs_'
                          + replaceSymbols(chan.variableName), 'h'
                          + sam.name + 'Nom_' + regionString + '_obs_'
                          + replaceSymbols(chan.variableName),
                         chan.nBins, chan.binLow, chan.binHigh)
                for iBin in xrange(self.hists['h' + sam.name + 'Nom_'
                                    + regionString + '_obs_'
                                    + replaceSymbols(chan.variableName)].GetNbinsX()
                                    + 1.):
                    self.hists['h' + sam.name + 'Nom_' + regionString
                                + '_obs_'
                                + replaceSymbols(chan.variableName)].SetBinContent(iBin
                             + 1., 1.)
            chan.getSample(sam.name).setHistoName('h' + sam.name
                     + 'Nom_' + regionString + '_obs_'
                     + replaceSymbols(chan.variableName))

            if not 'h' + sam.name + 'Nom_' + normString + 'Norm'\
                 in self.hists.keys():
                if self.readFromTree:
                    self.hists['h' + sam.name + 'Nom_' + normString
                                + 'Norm'] = TH1F('h' + sam.name + 'Nom_'
                             + normString + 'Norm', 'h' + sam.name
                             + 'Nom_' + normString + 'Norm', 1., 0.5,
                            1.5)
                    self.chains[self.prepare.currentChainName].Project('h'
                             + sam.name + 'Nom_' + normString + 'Norm',
                            normCuts, self.prepare.weights + ' * ('
                             + normCuts + ')')
                else:
                    self.hists['h' + sam.name + 'Nom_' + normString
                                + 'Norm'] = None
            ...
            for (systName, syst) in \
                chan.getSample(sam.name).systDict.items():
                print '    Systematic: %s' % systName

                if syst.type == 'weight' or syst.type == 'tree'\
                     or syst.type == 'user':
            ...
            self.addHistoSysForQCD(regionString, normString, normCuts,
                                   chan, sam)
        ...
        for (reg, high, low, nom) in userNormDict[(syst, sam)]:
            try:
                ...
                print 'ERROR: generating HistoSys for %s syst=%s nom=%g high=%g low=%g remove from fit.'\
                     % (nom, syst, nomIntegral, highIntegral,
                        lowIntegral)
    ...
    def makeDicts(self, chan):
        ...
        for (samName, infoList) in chan.infoDict.items():
            for info in infoList:
                ...
                if not info[0.] == 'Nom':
                    continue
                if not (info[0.], regString,
                        replaceSymbols(chan.variableName))\
                     in canDict.keys():
                    canDict[(info[0.], regString,
                            replaceSymbols(chan.variableName))] = \
                        TCanvas(
                        'c' + topLvl.name + '_' + info[0.] + '_'
                             + regString + '_obs_'
                             + replaceSymbols(chan.variableName),
                        'c' + info[0.] + '_' + regString + '_obs_'
                             + replaceSymbols(chan.variableName),
                        800,
                        600,
                        800,
                    ...
                    stackDict[(info[0.], regString,
                              replaceSymbols(chan.variableName))] = \
                        THStack(topLvl.name + '_' + regString + '_obs_'
                                 + replaceSymbols(chan.variableName)
                                 + 'Stack' + info[0.], '')

                    legDict[(info[0.], regString,
                            replaceSymbols(chan.variableName))] = \
                        TLegend(0.7, 0.7, 0.88, 0.88)
                    legDict[(info[0.], regString,
                            replaceSymbols(chan.variableName))].SetBorderSize(0.)
                    legDict[(info[0.], regString,
                            replaceSymbols(chan.variableName))].SetTextFont(42)
                    legDict[(info[0.], regString,
                            replaceSymbols(chan.variableName))].SetTextSize(0.05)
                    legDict[(info[0.], regString,
                            replaceSymbols(chan.variableName))].SetFillColor(0.)
                    legDict[(info[0.], regString,
                            replaceSymbols(chan.variableName))].SetLineColor(0.)

                    legDict[(info[0.], regString,
                            replaceSymbols(chan.variableName))].Clear()

                if not (regString, replaceSymbols(chan.variableName))\
                     in dataNameDict.keys():
                    dataNameDict[(regString,
                                 replaceSymbols(chan.variableName))] = \
                        ''
                if not (regString, replaceSymbols(chan.variableName))\
                     in qcdNameDict.keys():
                    qcdNameDict[(regString,
                                replaceSymbols(chan.variableName))] = ''

                if not topLvl.getSample(samName).isData\
                     and not topLvl.getSample(samName).isQCD:
                    self.hists['h' + samName + info[0.] + '_'
                                + regString + '_obs_'
                                + replaceSymbols(chan.variableName)].SetLineColor(topLvl.getSample(samName).color)
                    self.hists['h' + samName + info[0.] + '_'
                                + regString + '_obs_'
                                + replaceSymbols(chan.variableName)].SetFillColor(topLvl.getSample(samName).color)
                    stackDict[(info[0.], regString,
                              replaceSymbols(chan.variableName))].Add(self.hists['h'
                             + samName + info[0.] + '_' + regString
                             + '_obs_'
                             + replaceSymbols(chan.variableName)],
                            'hist')
                    legDict[(info[0.], regString,
                            replaceSymbols(chan.variableName))].AddEntry(self.hists['h'
                             + samName + info[0.] + '_' + regString
                             + '_obs_'
                             + replaceSymbols(chan.variableName)],
                            samName + info[0.], 'lf')
                elif topLvl.getSample(samName).isQCD:
                    self.hists['h' + samName + 'Nom_' + regString
                                + '_obs_'
                                + replaceSymbols(chan.variableName)].SetLineColor(topLvl.getSample(samName).color)
                    self.hists['h' + samName + 'Nom_' + regString
                                + '_obs_'
                                + replaceSymbols(chan.variableName)].SetFillColor(topLvl.getSample(samName).color)
                    self.hists['h' + samName + 'Low_' + regString
                                + '_obs_'
                                + replaceSymbols(chan.variableName)].SetLineColor(topLvl.getSample(samName).color)
                    self.hists['h' + samName + 'Low_' + regString
                                + '_obs_'
                                + replaceSymbols(chan.variableName)].SetFillColor(topLvl.getSample(samName).color)
                    self.hists['h' + samName + 'High_' + regString
                                + '_obs_'
                                + replaceSymbols(chan.variableName)].SetLineColor(topLvl.getSample(samName).color)
                    self.hists['h' + samName + 'High_' + regString
                                + '_obs_'
                    ...
                    qcdNameDict[(regString,
                                replaceSymbols(chan.variableName))] = \
                        'h' + samName + 'Nom_' + regString + '_obs_'\
                         + replaceSymbols(chan.variableName)
                else:
                    self.hists['h' + samName + '_' + regString + '_obs_'
                                + replaceSymbols(chan.variableName)].SetLineColor(topLvl.getSample(samName).color)
                    dataNameDict[(regString,
                                 replaceSymbols(chan.variableName))] = \
                        'h' + samName + '_' + regString + '_obs_'\
                         + replaceSymbols(chan.variableName)
        ...
        for info in stackDict.keys():
            ...
            if not qcdNameDict[(info[1.], info[2])] == '':
                stackDict[info].Add(self.hists[qcdNameDict[(info[1.],
                                    info[2])]], 'hist')
                legDict[info].AddEntry(self.hists[qcdNameDict[(info[1.],
                        info[2])]], 'QCDNom', 'lf')

            if not dataNameDict[(info[1.], info[2])] == '':
                legDict[info].AddEntry(self.hists[dataNameDict[(info[1.],
                        info[2])]], 'Data', 'lf')
                self.hists[dataNameDict[(info[1.],
                           info[2])]].SetStats(False)
                self.hists[dataNameDict[(info[1.],
                           info[2])]].GetYaxis().SetTitleOffset(1.3)
                self.hists[dataNameDict[(info[1.], info[2])]].Draw()
                self.hists[dataNameDict[(info[1.],
                           info[2])]].GetXaxis().SetTitle(chan.variableName)
                self.hists[dataNameDict[(info[1.],
                           info[2])]].GetYaxis().SetTitle('entries')
                stackDict[info].Draw('same')
                self.hists[dataNameDict[(info[1.],
                           info[2])]].Draw('same')

                if self.hists[dataNameDict[(info[1.],
                              info[2])]].GetBinContent(self.hists[dataNameDict[(info[1.],
                        info[2])]].GetMaximumBin())\
                     > stackDict[info].GetMaximum():
                    rangeMax = self.hists[dataNameDict[(info[1.],
                            info[2])]].GetBinContent(self.hists[dataNameDict[(info[1.],
                            info[2])]].GetMaximumBin())
                else:
                ...
                if self.hists[dataNameDict[(info[1.],
                              info[2])]].GetBinContent(self.hists[dataNameDict[(info[1.],
                        info[2])]].GetMinimumBin())\
                     < stackDict[info].GetMinimum():
                    rangeMin = self.hists[dataNameDict[(info[1.],
                            info[2])]].GetBinContent(self.hists[dataNameDict[(info[1.],
                            info[2])]].GetMinimumBin())
                else:
                ...
                self.hists[dataNameDict[(info[1.],
                           info[2])]].GetYaxis().SetRangeUser(rangeMin
                         - 0.1 * rangeMin, rangeMax + 0.1 * rangeMax)
            else:
                stackDict[info].Draw()
                ...
                stackDict[info].GetHistogram().GetYaxis().SetRangeUser(rangeMin
                         - 0.1 * rangeMin, rangeMax + 0.1 * rangeMax)
            ...
            if not os.path.isdir('plots/' + self.analysisName + '/'
                                  + topLvl.name):
                os.mkdir('plots/' + self.analysisName + '/'
                          + topLvl.name)
            canDict[info].SaveAs('plots/' + self.analysisName + '/'
                                  + topLvl.name + '/stack' + info[1.]
                                  + '_obs_' + info[2] + '_' + info[0.]
                                  + '.png')
    ...
    def outputRoot(self):
        ...
        if outputRootFile:
            ...
            def notNull(x):
                return not type(x).__name__ == 'TObject'
