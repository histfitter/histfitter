...
####### CONFIGURATION ####### 
configFile = 'CombinedKFactorFit_5Channel_GG_onestepCC_gridX.py'
...
## ### code to get list of samples:
## from ROOT import *
## file = TFile.Open("root://eosatlas//eos/atlas/atlascerngroupdisk/phys-susy/histfitter/stronglepton/SusyFitterTree_OneSoftMuo_SM_GG_onestepCC_LSP60_v3.root")#SusyFitterTree_OneSoftMuo_SM_GG_onestepCC_v3.root")

## allkeys = file.GetListOfKeys()
## next = ROOT.TIter(allkeys)
## ConvertList = []
## while True:
##     obj = next()
##     if not obj : break
##     if 'NoSys' in obj.GetName():
##         ConvertList.append(obj.GetName().replace("_NoSys",""))
...
logDir       = "%s/submit/logs/workspaceGeneration/%s"       % (SusyFitterDir,configFile.strip(".py"))
workspaceDir = "%s/submit/workspaces/%s" % (SusyFitterDir,configFile.strip(".py"))

os.system("mkdir -p %s " % (logDir))
os.system("mkdir -p %s " % (workspaceDir))
