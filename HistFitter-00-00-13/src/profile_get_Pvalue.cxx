...
void
Combination::doFreeFit( const RooWorkspace* fullwspace, RooDataSet* inputdata, bool verbose )
{
  RooFitResult* result = Combination::doFreeFitSave( fullwspace, inputdata, verbose );
  delete result;
...
//________________________________________________________________________________________________
RooFitResult*
Combination::doFreeFitSave( const RooWorkspace* fullwspace, RooDataSet* inputdata, bool verbose )
{
  // fit to reset the workspace

  if(!fullwspace){
     std::cerr << " Input workspace is null!" << std::endl;
     return 0;
  }
  if(!fullwspace->obj("text_suffix")){
     std::cerr << " NO suffix found, this workspace can not be automatically combined using this function. If possible recreate your single workspaces with newer code" << std::endl;
     return 0;
  }
