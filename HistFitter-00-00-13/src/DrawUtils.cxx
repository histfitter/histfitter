...
TH2D*
DrawUtil::triwsmooth( TTree* tree, const char* varstr, const char* name, const char* title, const char* cutstr, TH2D* inputHist)
{
  ...
  cout << "Plotting " << varstr << " with selection " << cutstr << "." << endl; 
  cout << "Selected " << nselect << " out of " << ntotal << " entries. Fraction = " << double(nselect)/ntotal << endl;
...
TH2F*
DrawUtil::makesignificancehistos( TTree* tree, int mode,TString id1,TString id2,int   nbinsX,int nbinsY, float minX,float maxX, float minY, float maxY)
{
  if (tree==NULL) return 0;
...
TH2F*
DrawUtil::linearsmooth(const TH2& hist, const char* name, const char* title)
{
